...
wc = wc.replace('<Bar dataKey="precip" fill="var(--chart-precip)" radius={[4, 4, 0, 0]} name="Úhnn Zrážkok" />', `<Bar dataKey="precip" radius={[4, 4, 0, 0]} name="Úhrn Zrážok">
                {hourlyData.map((entry, index) => (
                  <Cell key={\`cell-${index}\`} fill={entry.snowfall > 0 ? '#d4d4dc' : 'var(--chart-precip)'} />
                ))}
              </Bar>`);
